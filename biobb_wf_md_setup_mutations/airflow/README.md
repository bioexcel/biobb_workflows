...
* **biobb_wf_md_setup_mutations.py:** the **python DAG** file with all the steps to execute this workflow.
...
Both folders and the python file must be located into a **biobb_wf_md_setup_mutations folder** where the other **Apache Airflow Dags** are located (usually the **dags/** folder).

### <a name="utils"></a>Utils

* [**airflow_cwl_utils.py**](../../common/airflow/dags/airflow_cwl_utils.py): This file **generates the bash command** that will be executed for each step of the dag. It must be located into the main **dags/** folder, at the same level of the **biobb_wf_md_setup_mutations folder** created in the step before.
...
2) Launch the workflow:
        ...
        docker exec [CONTAINER ID] airflow dags trigger biobb_wf_md_setup_mutations
...
3) Monitor workflow status:
        ...
        docker exec [CONTAINER ID] airflow tasks states-for-dag-run biobb_wf_md_setup_mutations [dag_run_id]

### <a name="run-wf-i"></a>Run workflow via GUI

Go to **Dags list** in the main menu. Search for the **biobb_wf_md_setup_mutations dag**, click it and, once inside the dag, click on **Execute button** at the top right.
...
Once the workflow is finished, you just should enter the new **dags/biobb_wf_md_setup_mutations/outputs/** folder and, inside it, you will find a folder for each step of the workflow with all the files generated in every step.