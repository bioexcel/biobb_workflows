#!/usr/bin/env bash
#
# biobb_wf_amber_abc_setup — airflow flavour e2e test
#
...
#   <scratch>/dags/biobb_wf_amber_abc_setup/  (copy of <wf>/airflow)
...
# maxcyc -> 50 (the mdin overrides live in the JSON config strings of the
# per-step inputs/*.yml). The un-reduced workflow would run ~100 ps of
# production MD.
#
# Usage:
...
adjust_runtime() {
  # $1 = <scratch>/dags/<wf>/inputs — per-step yml files. Shorten every
  # sander step to the CI python values (the mdin overrides are JSON
  # strings inside the config: line of each file).
  ...
  "${SED_INPLACE[@]}" "s/\"maxcyc\": [0-9]*/\"maxcyc\": 50/g" "$1"/*.yml
  "${SED_INPLACE[@]}" "s/\"nstlim\": [0-9]*/\"nstlim\": 100/g" "$1"/*.yml
}
# --------------------------------------------------------------------------
