#!/usr/bin/env bash
#
# biobb_wf_ligand_parameterization — docker flavour e2e test
#
# Builds the workflow docker image from <wf>/docker/Dockerfile and runs it in
# MODE=python (default), mounting the committed input (ligand.pdb) read-only
# and keeping all outputs inside this test folder (tests/docker/work/), so the
...
# Final workflow outputs (step3_acpype_params_gmx in python/workflow.yml)
EXPECTED_OUTPUTS=(
  "output.params.gro"
  "output.params.itp"
  "output.params.top"
...
adjust_runtime() {
  # $1 = local workflow.yml used for the run. This wf is already fast; to make
  # the babel minimisation (step2) even faster you could relax its criteria:
  #   sed -i -E 's/(criteria: )1e-10/\1 1e-5/' "$1"
  :
}
# --------------------------------------------------------------------------
...
# (in workflow.yml), e.g. <work>/biobb_wf_ligand_parameterization/
