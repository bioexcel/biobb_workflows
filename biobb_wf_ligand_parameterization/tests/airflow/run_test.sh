#!/usr/bin/env bash
#
# biobb_wf_ligand_parameterization — airflow flavour e2e test
#
...
#   <scratch>/dags/biobb_wf_ligand_parameterization/  (copy of <wf>/airflow)
...
adjust_runtime() {
  ...
  # fast; to make the babel minimisation even faster you could relax criteria:
  #   sed -i -E 's/("criteria": ")[^"]+/\1 1e-5/' "$1"/step2_babel_minimize.yml
  :
}
# --------------------------------------------------------------------------
