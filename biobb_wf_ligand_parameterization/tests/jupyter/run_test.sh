#!/usr/bin/env bash
#
# biobb_wf_ligand_parameterization — jupyter flavour e2e test
#
...
# raises, so a clean run means every step worked (fetch ligand, add hydrogens,
# minimize, acpype). The script additionally asserts that the executed notebook
# contains no `error` outputs and that the final files exist and are non-empty.
#
...
# fetches the IBP structure from the MMB REST API, mmb.irbbarcelona.org).
#
# Usage:
#   ./run_test.sh [--no-build] [--pull] [--image TAG] [--keep]
#     --no-build   do not build the image (it must already exist locally)
#     --pull       pull ghcr.io/bioexcel/biobb_wf_ligand_parameterization:latest instead of building
#     --image TAG  image tag to build/use (default: biobb_wf_ligand_parameterization:test)
...
# <wf>/<wf>.ipynb under notebooks/) — workflow-specific line #1
NOTEBOOK="biobb_wf_ligand_parameterization/notebooks/biobb_wf_ligand_parameterization.ipynb"

# Final notebook outputs (the notebook names them <ligandCode>params.* with
# ligandCode=IBP) — workflow-specific line #2
EXPECTED_OUTPUTS=(
  "IBPparams.gro"
  "IBPparams.itp"
  "IBPparams.top"
...
execute_notebook_cmd() {
  ...
  # dir), so the relative outputs (IBP.pdb, IBP.H.mol2, IBPparams.*) land in
