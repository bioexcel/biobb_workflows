#!/usr/bin/env bash
#
# biobb_wf_ligand_parameterization — cwl flavour e2e test
#
...
# Final workflow outputs (step3_acpype_params_gmx)
EXPECTED_OUTPUTS=(
  "output.params.gro"
  "output.params.itp"
  "output.params.top"
...
adjust_runtime() {
  # $1 = workflow_input_descriptions.yml. This wf is already fast; to make the
  # babel minimisation (step2) even faster you could relax its criteria
  # (it lives inside the JSON config string):
  #   sed -i -E 's/("criteria": ")[^"]+/\1 1e-5/' "$1"
  :
}
# --------------------------------------------------------------------------
