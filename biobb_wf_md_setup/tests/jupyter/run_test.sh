#!/usr/bin/env bash
#
# biobb_wf_md_setup — jupyter flavour e2e test
#
...
# raises, so a clean run means every step worked (fetch 1AKI, pdb2gmx,
# solvate, minimize, nvt, npt, free MD, rmsd/rgyr, image). The script
# additionally asserts that the executed notebook contains no `error` outputs
# and that the final files exist and are non-empty.
#
# The notebook hard-codes its own nsteps (min 5000, nvt 5000, npt 5000,
# md 50000). adjust_runtime below reduces them to 10 in the local COPY only
# (same reduction as the CI python flavour) — the notebook in the jupyter
...
# fetches the 1AKI structure from the RCSB PDB database).
#
# Usage:
...
NOTEBOOK="biobb_wf_md_setup/notebooks/biobb_wf_md_setup.ipynb"

# Final notebook outputs (the notebook names them <pdbCode>_<step>.ext with
# pdbCode=1AKI, set in cell 4) — workflow-specific line #2
EXPECTED_OUTPUTS=(
  "1AKI_imaged_traj.trr"
  "1AKI_md_dry.gro"
)
...
adjust_runtime() {
  # $1 = the local copy of the notebook. Reduces every nsteps to 10 (same
  # reduction as the CI python flavour). The cells use both
  # 'nsteps': 5000 (int) and 'nsteps':'5000' (string) — one pattern for both.
...
execute_notebook_cmd() {
  ...
  # dir), so the relative outputs (1AKI_*.pdb/.trr/.xvg/...) land in the
  # mounted work dir.
