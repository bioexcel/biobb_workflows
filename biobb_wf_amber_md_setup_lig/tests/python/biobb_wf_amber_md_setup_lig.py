...
def step000_reduce_remove_hydrogens(config, system=None):
    ...
    assert fx.not_empty(global_paths["step000_reduce_remove_hydrogens"]["output_path"])
    assert fx.equal(global_paths["step000_reduce_remove_hydrogens"]["output_path"], f'reference/step000_reduce_remove_hydrogens/{Path(global_paths["step000_reduce_remove_hydrogens"]["output_path"]).name}')

    global global_work_dir
    global_work_dir = conf.get_working_dir_path()


def step00_extract_molecule(config, system=None):
    conf = settings.ConfReader(config, system)
    ...
    assert fx.not_empty(global_paths["step00_extract_molecule"]["output_molecule_path"])
    assert fx.equal(global_paths["step00_extract_molecule"]["output_molecule_path"], f'reference/step00_extract_molecule/{Path(global_paths["step00_extract_molecule"]["output_molecule_path"]).name}')


def step0000_cat_pdb(config, system=None):
    conf = settings.ConfReader(config, system)
    conf.working_dir_path = global_work_dir
    global_log, _ = fu.get_logs(path=conf.get_working_dir_path(), light_format=True)
    global_prop = conf.get_prop_dic(global_log=global_log)
    ...
    assert fx.not_empty(global_paths["step0000_cat_pdb"]["output_structure_path"])
    assert fx.equal(global_paths["step0000_cat_pdb"]["output_structure_path"], f'reference/step0000_cat_pdb/{Path(global_paths["step0000_cat_pdb"]["output_structure_path"]).name}')


def step0_cat_pdb(config, system=None):
    conf = settings.ConfReader(config, system)
    conf.working_dir_path = global_work_dir
    global_log, _ = fu.get_logs(path=conf.get_working_dir_path(), light_format=True)
    global_prop = conf.get_prop_dic(global_log=global_log)
    ...
    assert fx.not_empty(global_paths["step0_cat_pdb"]["output_structure_path"])
    assert fx.equal(global_paths["step0_cat_pdb"]["output_structure_path"], f'reference/step0_cat_pdb/{Path(global_paths["step0_cat_pdb"]["output_structure_path"]).name}')
...
def test_step000_reduce_remove_hydrogens(config_path, system):
    step000_reduce_remove_hydrogens(config_path, system)


@pytest.mark.parametrize("system", [None])
def test_step00_extract_molecule(config_path, system):
    step00_extract_molecule(config_path, system)


@pytest.mark.parametrize("system", [None])
def test_step0000_cat_pdb(config_path, system):
    step0000_cat_pdb(config_path, system)


@pytest.mark.parametrize("system", [None])
def test_step0_cat_pdb(config_path, system):
    step0_cat_pdb(config_path, system)
