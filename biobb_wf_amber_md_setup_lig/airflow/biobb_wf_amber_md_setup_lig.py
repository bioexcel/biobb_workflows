...
with DAG(WF_NAME, start_date=datetime(2026, 1, 1), schedule=None) as dag:
    ...
    step000_reduce_remove_hydrogens = BashOperator(
        task_id="step000_reduce_remove_hydrogens",
        bash_command=create_bash_command(WF_NAME, "step000_reduce_remove_hydrogens", "reduce_remove_hydrogens")
    )

    step00_extract_molecule = BashOperator(
        task_id="step00_extract_molecule",
        bash_command=create_bash_command(WF_NAME, "step00_extract_molecule", "extract_molecule")
    )

    step0000_cat_pdb = BashOperator(
        task_id="step0000_cat_pdb",
        bash_command=create_bash_command(WF_NAME, "step0000_cat_pdb", "cat_pdb")
    )

    step0_cat_pdb = BashOperator(
        task_id="step0_cat_pdb",
        bash_command=create_bash_command(WF_NAME, "step0_cat_pdb", "cat_pdb")
    )
    ...
    step000_reduce_remove_hydrogens >> step00_extract_molecule >> step0000_cat_pdb >> step0_cat_pdb >> step1_pdb4amber_run >> step2_leap_gen_top
